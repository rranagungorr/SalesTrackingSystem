...
public class Register extends PanelCustom implements ITriggerer {
    ...
    private void registerDealerButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_registerDealerButtonActionPerformed
        ...
        if (flagForCheckBox == true) {
            ...
            } else if (UserDatabase.getUserByUsername(txtUsername.getText()) != null) {
                JOptionPane.showMessageDialog(this, "Username Already Exists.",
                        "Failure", JOptionPane.INFORMATION_MESSAGE);

            } else if (flagForName && flagForEmail && flagForUsername && flagForPassword && flagForPhoneNumber) {
                Seller seller = new Seller(0, txtName.getText(), txtUsername.getText(), txtPassword.getText(),
                        txtEmail.getText(), txtPhoneNumber.getText());
                UserDatabase.addUser(seller); // Veritabanına ekle
    ...
    private void registerCustomerButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_registerCustomerButtonActionPerformed
        if (txtUsername.getText().isEmpty() || txtPassword.getText().isEmpty() || txtName.getText().isEmpty()
            ...
            Customer customer = new Customer(0, txtName.getText(), txtUsername.getText(), txtPassword.getText(),
                    txtEmail.getText(), txtPhoneNumber.getText());
            UserDatabase.addUser(customer); // Veritabanına ekle
